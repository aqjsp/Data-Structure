...
二分查找先写清循环不变量：区间是闭还是开，`mid` 落哪边。开闭一混就会死循环或漏边界。`mid = left + (right - left) / 2`，不要写成 `(left + right) / 2`（整型可能溢出）。

![二分：先写不变式再写代码](./image/binary-search.svg)

哈希平均 O(1) 的前提是均匀散列 + 装载因子受控。冲突处理两条路：链地址（Java HashMap 链表再树化）和开放寻址（Python dict）。最坏仍是 O(n)。

![哈希冲突：链地址 vs 开放寻址](./image/hash-chain.svg)
