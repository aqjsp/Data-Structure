...
回溯是在解空间树上 DFS：做选择 → 递归 → 撤销选择。要的是全部方案（或第一个合法方案），不是「某种最优递推」。剪枝越早越好：路径已经不合法就 return，不要等叶子。分支限界把同样的树改成 BFS / 优先队列，用界把不可能更优的枝裁掉。

![回溯：枚举 + 撤销；分支限界换队列](./image/dp-vs-greedy.svg)
