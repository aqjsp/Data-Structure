...
下边给大家画一下二叉树的图。先把几个容易混的词分开：满二叉树每层都满；完全二叉树只要求最后一层左对齐（堆用数组存就是它）；BST 是左 < 根 < 右，退化成链就是 O(n)；AVL / 红黑树用旋转把高度压在 O(log n)。堆是完全二叉树 + 堆序，不是 BST。

![满 / 完全 / BST / 平衡，别混](./image/binary-tree-types.svg)
