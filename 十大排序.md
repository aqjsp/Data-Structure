...
| 希尔排序 | O(n log n)～O(n²) | 取决于间隔序列 | 取决于间隔序列 | O(1)       | 不稳定 |
...
表里希尔那一行旧稿写成了 O(log² n)，那是错的。希尔是插入排序的分组版，最好常见 Hibbard 一类间隔大约 O(n^{3/2}) 到接近 O(n log n)，没有「O(log² n)」这种保证。基数排序是 d 轮稳定计数（或桶），d 是关键字位数。

面试手写优先级：快排、归并。快排平均快、原地、不稳定，最坏会退化，工程上要随机 pivot / 三数取中。归并始终 O(n log n)、稳定，但要 O(n) 额外空间，外排序和链表排序用它。

![快排 vs 归并：原地不稳定 vs 稳定要额外空间](./image/quick-merge.svg)
